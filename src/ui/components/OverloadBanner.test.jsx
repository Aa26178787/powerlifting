...
describe('OverloadBanner', () => {
  it('renders null when overload is absent', () => {
    const { container } = render(<OverloadBanner overload={null} liftLog={[]} />)
    expect(container.firstChild).toBeNull()
  })

  it('renders heading when overload present', () => {
    render(<OverloadBanner overload={overload} liftLog={[]} />)
  ...
  it('shows abort note when liftLog triggers detectOverreaching (declining readiness ≥3 entries)', () => {
    ...
    render(<OverloadBanner overload={overload} liftLog={log} />)
    expect(screen.getByText(/abort 권고/)).toBeInTheDocument()
  })

  it('does not show abort note when liftLog is empty', () => {
    render(<OverloadBanner overload={overload} liftLog={[]} />)
    expect(screen.queryByText(/abort 권고/)).not.toBeInTheDocument()
  })

  it('does not show abort note when liftLog is healthy', () => {
    const log = [
