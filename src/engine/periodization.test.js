...
describe('buildWorkingWeeks v3', () => {
  it('builds 3 weeks and tags every exercise with a quality + rep range', () => {
    const weeks = buildWorkingWeeks(getTemplate('dup').layouts[3], ctx)
  ...
  it('block model concentrates a single quality in week 1', () => {
    const weeks = buildWorkingWeeks(getTemplate('dup').layouts[3], { ...ctx, model: 'block' })
...
describe('e1rmModifier applied to weight', () => {
  it('a variation slot is lighter than the comp lift at the same quality/e1rm', () => {
    ...
    const weeks = buildWorkingWeeks(getTemplate('dup').layouts[3], { ...ctx, stickingPoint: { squat:'bottom', bench:'none', deadlift:'bottom' } })
    const exs = weeks.flatMap((w) => w.sessions).flatMap((s) => s.exercises)
...
describe('set schemes + overrides in working weeks', () => {
  it('every exercise carries a scheme with concrete sets', () => {
    const weeks = buildWorkingWeeks(getTemplate('dup').layouts[3], { ...ctx, advanced: true, totalWeeks: 3 }, 3)
  ...
  it('respects a totalWeeks of 5', () => {
    expect(buildWorkingWeeks(getTemplate('dup').layouts[3], { ...ctx, totalWeeks: 5 }, 5)).toHaveLength(5)
  })
  it('variationOverride forces the chosen variation name on its lift slots', () => {
    const weeks = buildWorkingWeeks(getTemplate('dup').layouts[3], { ...ctx, variationOverride: { squat: 'Front Squat', bench: null, deadlift: null } }, 3)
  ...
  it('attaches a tempo spec to tempo exercises', () => {
    const weeks = buildWorkingWeeks(getTemplate('dup').layouts[3], { ...ctx, variationOverride: { squat: 'Tempo Squat', bench: null, deadlift: null } }, 3)
  ...
  it('a cue deficit prescribes its teaching variation on a variation slot (4-day has a deadlift variation)', () => {
    const weeks = buildWorkingWeeks(getTemplate('dup').layouts[4], { ...ctx, cueNeed: { squat: null, bench: null, deadlift: 'legDrive' } }, 3)
  ...
  it('ignores a variationOverride that is in excludedExercises', () => {
    const weeks = buildWorkingWeeks(getTemplate('dup').layouts[3], {
      ...ctx,
...
describe('buildWorkingWeeks concurrent (mixed blend)', () => {
  ...
  it('emits a strengthHypertrophy main exercise with both rep ranges', () => {
    const weeks = buildWorkingWeeks(getTemplate('dup').layouts[4], ctx(), 4)
